...
function getHairMask(
  ...
  for (let index = 0; index < pixels.data.length; index += 4) {
    ...
    // The uploaded sheet background is #FEFEFF rather than pure white.
    // Leaving its 1px alpha behind creates a visible rectangular halo on
    // the dark plaza background.
    pixels.data[index + 3] = isSheetBackground
      ? 0
      : Math.max(0, 255 - luminance);
  }
