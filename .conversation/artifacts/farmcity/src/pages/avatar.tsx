...
function AccessoryThumbnail({
  ...
  return (
    <canvas
      ...
      height={86}
      className="wardrobe-hair-thumbnail"
      data-testid={`canvas-accessory-thumbnail-${accessory}`}
      aria-hidden="true"
    />
  );
}
...
export default function AvatarCreator() {
  const { token, player, setPlayer, logout } = useAuth();
  ...
  const filteredAccessories = accessories.filter((item) =>
    (ACCESSORY_STYLE_LABELS[item] ?? item).toLowerCase().includes(search.trim().toLowerCase()),
  );
  ...
  const handleCancel = () => {
    if (player?.avatar || existingAvatar) {
      setLocation('/plaza');
      return;
    }
    logout();
  ...
  <div className="wardrobe-title-decoration">
                  ...
                  {filteredAccessories.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        key={item}
                        className={`wardrobe-item ${selected ? 'wardrobe-item-selected' : ''}`}
                        data-testid={`button-accessory-${item}`}
                        aria-pressed={selected}
        ...
        <footer className="wardrobe-footer">
          ...
          >
            <X size={19} />
             {player?.avatar || existingAvatar ? 'Cancelar' : 'Salir'}
          </button>
          <button
            type="button"
