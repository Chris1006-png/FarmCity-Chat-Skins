...
export function AuthWrapper({ children }: { children: React.ReactNode }) {
  ...
  // A player returned by login/register is already usable. A temporary
  // /auth/me failure should not blank the game after a successful sign-in;
  // only block the initial session restore where there is no player yet.
  if (token && error && errorStatus !== 401 && !player) {
    return (
      <div
