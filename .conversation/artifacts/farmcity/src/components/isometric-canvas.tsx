...
function mapSizeForRoom(roomTiles: Array<{ x: number; y: number }>): number {
  if (roomTiles.length === 0) return DEFAULT_MAP_SIZE;
  const maxCoordinate = roomTiles.reduce(
    (max, tile) => Math.max(max, tile.x + 1, tile.y + 1),
    DEFAULT_MAP_SIZE,
  );
...
function getTileColor(x: number, y: number, textureId: string): string {
  const colors = roomTextureColors(textureId);
  if (textureId.includes('grass')) {
    return (x + y) % 2 === 0 ? colors.base : colors.light;
  }
  return (x + y) % 2 === 0 ? colors.base : colors.dark;
}
...
function drawTile(
  ...
  if (isTarget && highlightAge < 2.5) {
    ...
    ctx.fillStyle = getTileColor(x, y, floorTextureId);
  }
  ctx.fill();
...
export function IsometricCanvas({
  ...
  roomTiles, roomWalls, roomFloorTextureId,
  ...
  const roomFloorTextureIdRef = useRef(roomFloorTextureId ?? 'floor-wood');
  ...
  useEffect(() => {
    roomFloorTextureIdRef.current = roomFloorTextureId ?? 'floor-wood';
  }, [roomFloorTextureId]);
  useEffect(() => {
  ...
  const handlePointer = useCallback((clientX: number, clientY: number) => {
    ...
    const roomTiles = roomTilesRef.current;
    const mapSize = mapSizeForRoom(roomTiles);
    if (tx < 0 || tx >= mapSize || ty < 0 || ty >= mapSize) return;
    const roomSet = new Set(roomTiles.map((tile) => `${tile.x},${tile.y}`));
    ...
    const blocked = roomSet.size > 0
      ? new Set(Array.from({ length: mapSize }, (_, x) =>
          Array.from({ length: mapSize }, (_, y) => `${x},${y}`),
        ).flat().filter((key) => !roomSet.has(key)))
      : OBSTACLES;
    const path = aStar({ x: sx, y: sy }, { x: tx, y: ty }, blocked, mapSize);
    if (path.length > 0) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (heldKeysRef.current.size > 0 && pathRef.current.length === 0) {
        ...
        if (kx !== 0 || ky !== 0) {
          ...
          const blocked = roomSet.size > 0
            ...
            : OBSTACLES;
          let nx = Math.max(0, Math.min(mapSize - 1, cx + kx));
          let ny = Math.max(0, Math.min(mapSize - 1, cy + ky));

          // Diagonal obstacle fallback: try each cardinal axis independently
          if (blocked.has(`${nx},${ny}`) && kx !== 0 && ky !== 0) {
            const xBlocked = blocked.has(`${Math.max(0,Math.min(mapSize - 1,cx+kx))},${cy}`);
            const yBlocked = blocked.has(`${cx},${Math.max(0,Math.min(mapSize - 1,cy+ky))}`);
            if (!xBlocked) { ny = cy; }
      ...
      const mapSize = mapSizeForRoom(savedRoom);
      ...
      for (let sum = 0; sum < mapSize * 2 - 1; sum++) {
        for (let x = 0; x <= sum; x++) {
          const y = sum - x;
          if (x >= mapSize || y >= mapSize) continue;
          if (savedRoomSet.size > 0 && !savedRoomSet.has(`${x},${y}`)) continue;
          const { sx, sy } = isoToScreen(x, y);
          drawTile(ctx, sx, sy, x, y, roomFloorTextureIdRef.current, targetTile, highlightAge);
        }
      }
