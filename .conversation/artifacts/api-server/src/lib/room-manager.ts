...
export function normalizeRoomGeometry(
  tiles: RoomTile[],
  walls: RoomWall[],
): { tiles: RoomTile[]; walls: RoomWall[] } {
  if (tiles.length === 0) return { tiles, walls };
  const minX = Math.min(...tiles.map((tile) => tile.x));
  const minY = Math.min(...tiles.map((tile) => tile.y));
  return {
    tiles: tiles.map((tile) => ({ x: tile.x - minX, y: tile.y - minY })),
    walls: walls.map((wall) => ({ ...wall, x: wall.x - minX, y: wall.y - minY })),
  };
}
...
export const DEFAULT_PLAZA_ROOM_ID = "plaza";

const DEFAULT_PLAZA_TILES: RoomTile[] = Array.from({ length: 12 }, (_, y) =>
  Array.from({ length: 12 }, (_, x) => ({ x, y })),
).flat();

export function ensureDefaultPlaza(): void {
  if (roomManager.get(DEFAULT_PLAZA_ROOM_ID)) return;
  roomManager.add({
    id: DEFAULT_PLAZA_ROOM_ID,
    ownerId: 0,
    name: "Plaza central",
    tiles: DEFAULT_PLAZA_TILES,
    walls: [],
    doorPosition: { x: 6, y: 6 },
    floorTextureId: "floor-grass",
