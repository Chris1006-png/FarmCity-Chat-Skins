...
function spawnAtRoomEntry(client: GameClient, room: Room): void {
  const spawn = room.tiles.find(
    (tile) => tile.x === room.doorPosition.x && tile.y === room.doorPosition.y,
  ) ?? room.tiles[Math.floor(room.tiles.length / 2)];
  if (!spawn) return;
  client.posX = spawn.x;
  client.posY = spawn.y;
}
...
async function loadRoom(roomId: string): Promise<Room | undefined> {
  if (roomId === DEFAULT_PLAZA_ROOM_ID) {
    ensureDefaultPlaza();
    return roomManager.get(DEFAULT_PLAZA_ROOM_ID);
  }
  ...
  const geometry = normalizeRoomGeometry(
    stored.tiles,
    stored.walls as Room["walls"],
  );
  const room: Room = {
    ...stored,
    tiles: geometry.tiles,
...
export function createWebSocketServer(server: import("http").Server): WebSocketServer {
  ...
  wss.on("connection", async (ws: WebSocket, req: IncomingMessage) => {
    ...
    const [pos] = await db
      ...
      .limit(1);
    const posX = pos?.posX ?? 6;
    const posY = pos?.posY ?? 6;
    ...
    messageHandler = async (raw) => {
        ...
        if (msg.type === "room:update") {
          ...
          if (updatedRoom) {
            broadcastToRoom(
              ...
              room.id,
              clients,
              roomMessage("room:state", roomManager.snapshot(updatedRoom)),
            );
          }
      ...
      if (msg.type === "room:join") {
        ...
        if (
          !room.isPublic &&
          !isRoomOwner &&
          room.password !== (typeof data.password === "string" ? data.password : undefined)
        ) {
        ...
        if (currentRoomId) {
          ...
          // Positions are persisted globally, not per room. Spawn at the
          // room entrance whenever a player changes rooms so a plaza
          // coordinate cannot place them outside a smaller custom room.
          spawnAtRoomEntry(client, room);
      ...
      if (
        ...
        // Keep sub-tile precision for remote interpolation, but never accept
        // a coordinate whose rounded tile is outside the current room.
        const roomSize = Math.max(
          1,
          currentRoom.tiles.reduce((max, tile) => Math.max(max, tile.x + 1, tile.y + 1), 0),
        );
        const newX = Math.max(0, Math.min(roomSize - 1, rawX));
        const newY = Math.max(0, Math.min(roomSize - 1, rawY));
        const roundedTile = `${Math.round(newX)},${Math.round(newY)}`;
        if (!currentRoom.tiles.some((tile) => `${tile.x},${tile.y}` === roundedTile)) return;
        ...
        // immediately to the current room; they do not belong in the
