...
const FLOOR_TEXTURES = [
  ...
  { id: 'floor-grass', name: 'Pasto verde', tone: 'grass' },
];
...
export default function RoomEditor() {
  ...
  const editorModeRef = useRef<EditorMode>(isNewRoom ? 'create' : 'edit');
  const passwordRef = useRef('');
  const [editorMode, setEditorMode] = useState<EditorMode>(isNewRoom ? 'create' : 'edit');
  ...
  const [wallTextureId, setWallTextureId] = useState('wall-plaster');
    ...
    mutation: {
      onSuccess: async (room) => {
        await Promise.all([
          ...
          queryClient.invalidateQueries({ queryKey: getGetMyRoomByIdQueryKey(room.id) }),
          queryClient.refetchQueries({ queryKey: getGetMyRoomsQueryKey(), type: 'all' }),
          queryClient.refetchQueries({ queryKey: getGetPublicRoomsQueryKey(), type: 'all' }),
        ]);
        setIsSaving(false);
        setFeedback({
          kind: 'success',
          title: 'Sala creada',
          message: `Se guardó correctamente. ID: ${room.id}`,
        });
        setLocation(`/rooms?created=${encodeURIComponent(room.id)}`);
      },
      onError: (error) => {
        setIsSaving(false);
        setFeedback({
          ...
          message: error instanceof Error ? error.message : 'El servidor rechazó la sala.',
        });
      },
    },
  });
  const { data: savedRoom } = useGetMyRoom({
    query: {
      enabled: !!token && !roomIdFromUrl,
      queryKey: getGetMyRoomQueryKey(),
      retry: false,
    },
  });
  const selectedRoomQuery = useGetMyRoomById(roomIdFromUrl ?? '', {
    query: {
      enabled: !!token && !isNewRoom && !!roomIdFromUrl,
      queryKey: getGetMyRoomByIdQueryKey(roomIdFromUrl ?? ''),
      retry: false,
    },
  });
  const selectedRoomById = selectedRoomQuery.data;
  const selectedRoom = roomIdFromUrl ? selectedRoomById : savedRoom;
  ...
  useEffect(() => {
    const nextMode: EditorMode = isNewRoom ? 'create' : 'edit';
  ...
  }, [isNewRoom, roomIdFromUrl, selectedRoomById, selectedRoomQuery.isLoading]);
  ...
  ws.onmessage = async (event) => {
      try {
        ...
        if (
          ...
          // The server acknowledgement is authoritative: it contains the new
          // room's unique ID that the list and playable view will use.
          setRoomId(message.data.roomId);
          setIsSaving(false);
          await Promise.all([
            queryClient.invalidateQueries({ queryKey: getGetMyRoomQueryKey() }),
            queryClient.invalidateQueries({ queryKey: getGetMyRoomByIdQueryKey(message.data.roomId) }),
            queryClient.refetchQueries({ queryKey: getGetMyRoomsQueryKey(), type: 'all' }),
            queryClient.refetchQueries({ queryKey: getGetPublicRoomsQueryKey(), type: 'all' }),
          ]);

          if (message.type === 'room:created' && editorModeRef.current === 'create') {
            setFeedback({
              kind: 'success',
              title: 'Sala creada',
  ...
  function handleSave() {
    // Only update a room that is actually loaded. When the account has no
    // rooms yet, selectedRoom is empty and this must always be a create.
    // This also prevents a stale in-memory roomId from causing ROOM_NOT_FOUND.
    const targetRoomId = editorMode === 'create'
      ? null
      : selectedRoom?.id ?? (roomIdFromUrl || null);
          ...
          <section className="room-editor-card room-editor-canvas-card" aria-labelledby="room-editor-title">
            <div className="room-editor-card-heading">
              <div>
                ...
                <p className="room-editor-subtitle">Cada toque cambia el suelo y puedes ver el resultado en tiempo real.</p>
              </div>
